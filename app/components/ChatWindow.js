...
export default function ChatWindow({ selectedUser, currentUser, darkMode, onMessageNotification }) {
  ...
  const sendVoiceMessage = async () => {
    ...
    try {
      setLoading(true);
      const extension = recordedAudio.mimeType?.includes('ogg') ? 'ogg' :
        recordedAudio.mimeType?.includes('mp4') ? 'm4a' : 'webm';
      const filePath = `voice-messages/${currentUser.uid}/${Date.now()}-${Math.random().toString(36).slice(2)}.${extension}`;
      const fileRef = storageRef(storage, filePath);

      await uploadBytes(fileRef, recordedAudio.blob, {
        contentType: recordedAudio.mimeType || 'audio/webm'
      });
      ...
      alert(error?.message || 'Unable to send voice message. Check your Firebase Storage rules and try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: bgColor }}>
                ...
                {msg.type === 'text' ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                    <audio
                      ...
                      src={msg.audioUrl || msg.fileUrl || (msg.audioData ? `data:audio/wav;base64,${msg.audioData}` : undefined)}
                    />
      ...
      {showPreview && recordedAudio && (
        <div style={{
          ...
          <span style={{ fontWeight: 700, color: textColor }}>{formatTime(recordedAudio.duration)}</span>
          <audio controls src={recordedAudio.url} style={{ flex: 1, minWidth: 0, height: 40 }} />
          <button onClick={discardPreview} disabled={loading} style={{ width: 40, height: 40, borderRadius: '50%', border: 'none', background: '#e74c3c', color: 'white', cursor: 'pointer' }} title="Delete recording">✕</button>
          <button onClick={sendVoiceMessage} disabled={loading} style={{ width: 44, height: 44, borderRadius: '50%', border: 'none', background: '#667eea', color: 'white', cursor: loading ? 'not-allowed' : 'pointer', fontSize: '1.1rem', opacity: loading ? 0.6 : 1 }} title="Send voice message">➤</button>
        </div>
      )}
