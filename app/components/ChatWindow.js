...
export default function ChatWindow({ selectedUser, currentUser, darkMode, onMessageNotification }) {
  ...
  const startRecording = async (event) => {
    ...
    try {
      ...
      if (event?.currentTarget) micButtonRef.current = event.currentTarget;
      recordingPointerRef.current = event?.pointerId ?? null;
      recordingStartYRef.current = event?.clientY ?? null;
  ...
  const sendVoiceMessage = async () => {
    ...
    try {
      ...
      // Free-plan implementation: store the recording directly in
      // Realtime Database as a complete data URL.
      ...
      if (!audioData || typeof audioData !== 'string') {
        throw new Error('Could not prepare the voice recording.');
      }
  ...
  // Pointer gesture: release to stop, swipe up at least 80px to lock.
  const handleMicPointerMove = (event) => {
    if (!isRecording || isLocked || recordingStartYRef.current == null) return;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: bgColor }}>
      ...
      {/* =========================
          DESKTOP VOICE PREVIEW
          ========================= */}
      {!isMobile && showPreview && recordedAudio && (
        <div style={{
          position: 'fixed',
          left: 0,
          right: 0,
          ...
          <span style={{ fontWeight: 700, color: textColor, minWidth: 38 }}>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{
                    ...
                    {(waveformData.length ? waveformData : Array(28).fill(5)).map((val, idx) => (
                      <span
                        ...
                        style={{
                          width: 2,
                          height: `${Math.max(3, Math.min(24, val / 5))}px`,
                          background: '#667eea',
                          borderRadius: 2
                        }}
                      />
                ...
                <div style={{
                  ...
                  {isLocked
                    ? '🔒 Recording locked — tap pause or finish'
                    : '↑ Slide up to lock'}
                </div>
              </div>
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    color: 'white',
                    fontSize: '1.15rem',
                    opacity: loading || !newMessage.trim() ? 0.45 : 1
                  }}
                >
                  ➤
                </button>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      {(waveformData.length ? waveformData : Array(22).fill(5)).map((val, idx) => (
                        <span
                          key={idx}
                          style={{
                            width: 3,
                            minWidth: 3,
                            height: `${Math.max(4, Math.min(26, val / 5))}px`,
